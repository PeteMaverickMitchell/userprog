...
/* list of processes in block state. */
...
thread_sleep (int64_t ticks)
{
  ...
  /* stores tick to wake up */
  now->wakey_ticks = ticks;
  /* add to sleep_list */
  list_insert_ordered(&sleep_list, &now->elem, cmp_thread_ticks, NULL);
  /* makes current thread go to sleep */
  thread_block();

  /* changes the status of interrupt to normal */
...
thread_wakeup (int64_t current_ticks)
{
  ...
  while(now_elem != list_end(&sleep_list))
  {
    ...
    if(current_ticks >= now_thread->wakey_ticks)
    {
      now_elem = list_remove(now_elem);
      /* remove from sleep_list and puts next elem in now_elem */
...
cmp_thread_ticks(const struct list_elem*a, const struct list_elem*b, void*aux UNUSED)
{
  ...
  return st_a->wakey_ticks< st_b->wakey_ticks;
}
